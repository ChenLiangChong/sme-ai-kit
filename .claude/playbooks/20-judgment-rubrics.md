...
1. **測試綠**（程式類改動）：跑了本次改動對應的測試 + 全量回歸（`.venv/bin/python3 -m pytest mcp-servers/business-db/tests/ -q`），宣稱時附數字（幾 passed）。純文件改動沒有對應測試，以條 2 的 read-back 取代本條。
...
| 老闆規則 / 決策寫入 | `store_fact` 必帶 `source_quote`（老闆原話）、推斷標 `source_type='inferred'`——絕不把推斷偽裝成老闆指示；對外行銷內容必走 HITL 審核 |
