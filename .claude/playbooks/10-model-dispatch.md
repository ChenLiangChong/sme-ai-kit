...
| 法條 / 期間天數查證 | 主對話直接用 `taiwan-legal-db` MCP（`query_regulation`） | — 反捏造鐵則：不寫記憶值 |
...
- **程式類產出**：跑測試或實跑。本 repo 全量回歸：`mcp-servers/business-db/tests/run_all.sh`；輸出落檔、主對話只看失敗清單。「測試都綠」的宣稱必須附測試輸出的數字（幾 passed）。
...
| 產品行為驗證 | 本 worktree `.mcp.json` 的 business-db 已指 dev DB(`data/business.db`);跨 repo e2e 另有 `/mnt/d/pm-scratch/sme_sandbox.db`;**絕不指 master 的 dogfood DB**(`/mnt/d/gitDir/sme-ai-kit/data/business.db`) |
| pleading 整合測試 | `.mcp.json` 已掛 `pleading` 薄 adapter(指 `lawyer-kg-product/pleading-manager/mcp_adapter/server.py`);後端要先在跑(Windows exe / `server_main.py`)、WSL→Windows 走 `172.19.48.1:8200`;token 走 `PLEADING_SESSION_TOKEN` 環境變數、祕密不入 git、不入 bridge/KB |
