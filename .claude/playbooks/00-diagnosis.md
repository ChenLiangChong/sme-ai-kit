...
> 立檔：2026-07-06（Fable 5 立制 session）；2026-07-07 自 legal-admin 線移植為 master 通用版（兩線各自演化、之後只能 cherry-pick）。讀者：之後在本 repo 開發的每一個 Claude session（主力 Opus、subagent 多為 Sonnet/Haiku）。
...
- dev 模式：SessionStart 只注入幾行開發紀律（指向本目錄）、UserPromptSubmit 靜默（另一支 `inject-line-routing.py` hook 是條件式、只在 prompt 含 LINE channel 訊息時才輸出，dev 日常同樣無聲）。ops 模式：原文照舊、行為零變（master 的 ops 文字本來就是 SME 版）。
- `CLAUDE.md` 開頭新增〈Session 模式（先判別、再往下讀）〉短段。
- **判準**：你在 repo root 改程式/文件/測試 = dev；你在跑 dogfood 營運 readout、回 LINE 訊息、被 `start-line.sh` 啟動 = ops。dev session 不跑營運啟動、不寫 dogfood DB（`data/business.db` 是真實營運資料；驗證產品行為用 scratch DB、`SME_DB_PATH` 另指）。
...
**證據**：
- 本 repo 的 `.mcp.json` 掛 7 個 server（apify / social / notionApi / fal-ai / agent-bridge…），營運（LINE / 社群 / 行銷）需要、純開發 session 用不到大半；apify 一個 MCP 進程 ~130MB，2026-06-07 併發 `claude -p` fan-out 實測凍死 8GB WSL。
- 大檔直讀案例：`mcp-servers/business-db/server.py` 與各 `modules/*/service.py`（千行級）、測試檔、codex 審查輸出（曾有 2812 行）整檔進主對話。

**修法**：
1. **不要再擴大工具面積**：本 repo `.mcp.json` 服務 dogfood 營運（line / social 等）、不動它，但別再往上加 server；單發 `claude -p` 一律 `--strict-mcp-config --mcp-config '{"mcpServers":{}}'` 砍 MCP；重讀取的開發工作照修法 2 派 subagent、別靠主對話硬扛（`.mcp.json` 永遠不進 git、per-project 生成）。
...
- **文件-程式漂移 = 慢性反捏造**：本產品的文件（CLAUDE.md / skills）宣稱的能力若超前程式 = 使用者以為有保護、實際沒有。改文件前先對程式碼驗證該能力真的存在；發現漂移優先修文件、不要「順手」補程式。
