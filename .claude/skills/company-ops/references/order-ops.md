...
pending → confirmed → (QC passed) → shipped → delivered → paid
  │                      │              │            │
  └→ cancelled           │              │            └→ returned
                         │              └→ (re-QC passed) → 補出貨 fulfill_order
                         └→ QC failed → 通知主管
```

DB 支援的訂單狀態：`pending` | `confirmed` | `shipped` | `delivered` | `paid` | `cancelled` | `returned`
...
4. **建立訂單**（`create_order` 內建審核門檻檢查）
   ...
   - 超過門檻 → `create_order` **系統自建審核並上報簽核人**（系統自建審核、比照記帳機制），**訂單不會建立**、回「已自動建立審核 #N」。**不要 agent 手寫 `create_approval`**（系統用完整 `resume_params` 自建，確保核准後 gate 一字不差比對得過）。核准後由全權限 / 業務層帶 `approved_id` 呼叫 `create_order`，系統依審核鎖定的原始參數（customer_id / items_json / business_unit）消費、無需人工重填（gate-backed HITL A 類，見 CLAUDE.md〈HITL 審核〉）；通知主管由系統上報、非自行 reply
...
### 前置條件
- QC result = `passed`（全部出貨）或 `partial`（部分出貨，需指定品項）
...
1. **出貨確認**
   ...
   **部分出貨**（QC partial，主管核准後）：
   ```
   fulfill_order(
     order_id,
     partial_items_json='[{"sku":"A001","qty":5}, {"sku":"A003","qty":10}]'
   )
   ```
...
使用 `cancel_order` 工具自動處理（回補庫存 + 作廢帳款）：

| 階段 | 處理方式 |
|------|---------|
| pending/confirmed（未出貨） | `cancel_order(order_id, reason='原因')` — 直接取消 |
| shipped（已出貨未到貨） | 通知物流攔截 → 攔截成功 → `cancel_order(order_id, reason='物流攔截')` |
| delivered（已到貨） | 走退貨流程（見下方） |
...
- 大額訂單超門檻時 `create_order` **系統自建審核並上報**（比照記帳的自建審核機制）、**訂單不建立**、回「已自動建立審核 #N」——不要手寫 `create_approval`；核准後帶 `approved_id` 重呼 `create_order`、系統依鎖定參數消費（見第二節第 4 步）
...
- 不要用 `update_order(status='cancelled')` 取消已出貨訂單（用 `cancel_order`，否則庫存和帳款不會自動還原）

## 快速參考

### 建單→出貨完整流程
1. `find_customer(query='客戶名')` — 取得 customer_id、discount_rate、payment_terms
2. `check_stock(sku_or_name='A200', business_unit='brand_d')` — 確認庫存足夠
3. `create_order(customer_id=ID, items_json='[{"sku":"A200","name":"品名","qty":10,"price":350}]', business_unit='brand_d')` — 內建門檻檢查，超額會提示先審核
...
- **出貨前必須過 QC**，不可跳過
- **大額訂單超門檻時 `create_order` 系統自建審核並上報**（gate-backed HITL A 類）、訂單不建立、回「已自動建立審核 #N」——不要手寫 `create_approval`；核准後帶 `approved_id` 重呼 `create_order`，系統依鎖定的原始參數消費（見第二節第 4 步及 CLAUDE.md〈HITL 審核〉A 類）
