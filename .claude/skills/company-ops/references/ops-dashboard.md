...
5. `list_pending_leave_requests()` — 待簽請假申請（無資料時自然回 empty、不影響流程；輸出區塊顯示 0 件即可）
6. `monthly_summary()` — 本月收支（CLAUDE.md 啟動 readout 要求項；`get_context_summary` 本身不含這段）
...
| 待簽請假等 >= 3 天 | `list_pending_leave_requests` 自動於 label 附加「已等 N 天」 | 「{員工} {假別} 申請已等 N 天，請盡快處理」（>= 7 天視為緊急） |
| 知識庫矛盾 | `lint_knowledge(checks='contradictions')` 有結果 | 「知識庫有 X 組潛在矛盾，建議檢視」 |
