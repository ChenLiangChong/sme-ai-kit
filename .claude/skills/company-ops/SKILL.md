...
- **actor 身份**：floored session 的 actor 由系統取 line-channel 驗過的 `user_id`、**agent 自填的值會被忽略**；不可逆動作（刪除 / 離職）應具名 actor（強制拒空 + admin-gate 屬 #10、**目前未 enforce**）（見 CLAUDE.md〈actor 身份信任〉）。
