...
- **actor 身份**：floored session 的 actor 由系統取 line-channel 驗過的 `user_id`、**agent 自填的值會被忽略**；不可逆動作具名 actor + 權限關卡已落實：`update_employee` 需 admin、`delete_transaction` 需 manager、audit 記 verified 操作者名，operator（無 floor）為全權限路徑（見 CLAUDE.md〈actor 身份信任〉）。
