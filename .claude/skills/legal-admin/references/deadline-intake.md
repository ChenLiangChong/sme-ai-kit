...
1. **文書類型** → 對應 `type`：
   ...
   - **保全命起訴裁定**（假扣押／假處分，法院命債權人「於一定期間內起訴」）→ `type='provisional_litigation'`（裁定期間、見下方「保全命起訴」）
   - **訴願決定書 → 行政訴訟撤銷訴訟**（送達後 2 個月不變期間）→ `type='admin_revocation'`（程序月期間、見下方「行政訴訟起訴期間」）
...
- **建錯／已無意義的時限（建錯案、建錯型別、案件和解）→ `cancel_deadline(deadline_id, reason="<為何取消>", cancelled_by="<操作者>")`**：狀態轉 cancelled、cron 與掃描不再提醒、寫 `deadline_audit`、自動通報主持律師、同步 pleading 鏡像。已遞交（filed）不可直接取消——先 `unmark_deadline_filed` 撤銷誤標再取消（遞交事實不以取消抹除）。**取消不可逆**：誤取消請重新 `create_deadline` 建新筆重算，不要嘗試改回。注意：填錯送達日/天數是走 `amend_deadline` 重算、不是取消重建。
...
## 保全命起訴（假扣押／假處分、`type='provisional_litigation'`）

本案尚未繫屬時，債務人聲請、法院裁定**命債權人「於一定期間內起訴」**（民訴§529Ⅰ，假處分準用§533）。
**逾期未起訴 → 債務人得聲請撤銷假扣押／假處分裁定（§529Ⅳ）＝保全盡失、無補正餘地**（故 severity=red）。
關鍵與限期補正同理（court_set）：

- **「一定期間」是法院在命起訴裁定主文當下訂的、不是法定固定 30 日**（坊間「30 日」是慣例、非法律值＝
  捏造風險）。你**必須讀那紙命起訴裁定主文抓出天數**填 `statutory_days`，引擎不預設不臆測。
- `statutory_basis` 填**命起訴裁定文號 + 民訴§529**（非純法條）。`type='provisional_litigation'` 自動帶
  `period_type='court_set'`、`severity=red`、描述、觸發語「命起訴裁定送達」、**一律強制 `[需人工複核]`**。
- 回覆會提醒特例**§529Ⅲ**（夫妻剩餘財產差額分配假扣押：自「宣告改用分別財產制裁定**確定之日**」起 10 日內
  起訴——起算為「裁定確定」非送達、也走本 court_set 路徑由你讀裁定手填 10 日）。

```
create_deadline(
  matter_id=<案件ID>, type="provisional_litigation",
  service_base_date="2026-06-01",                      # 命起訴裁定送達日
  statutory_days=<讀裁定主文那個「○日內起訴」>,          # ← 必填！非固定30日
  statutory_basis="北院114全字第5號裁定 + 民訴§529",     # ← 必填！裁定文號
  created_by="<操作者>", confirm_intake_id=<intake_id>,
)
```
...
## 行政訴訟起訴期間（撤銷訴訟、`type='admin_revocation'`）

收到**訴願決定書**、要算提起行政訴訟（撤銷訴訟／課予義務訴訟）的起訴期間：行訴§106Ⅰ「**訴願決定書送達後
二個月之不變期間**」。與消滅時效都是「月」期間、但**性質不同**——這是**程序不變期間**、走送達/在途/末日順延
（行訴§88 期間依民法計算）：

- `type='admin_revocation'` 自動帶 `period_unit='month'` / `period_value=2` / `period_type='peremptory'`
  （不變期間）/ `statutory_basis='行政訴訟法§106Ⅰ'`、觸發語「訴願決定書送達」。
- `service_base_date` 填**訴願決定書送達日**（確定事實、非法律判斷）→ 起算次日、依曆 2 個月（民§121
  相當日前一日 / §121但書月末）、末日順延§122。**起算用送達日這個確定事實，故不像消滅時效那樣一律強制複核**
  （日曆完整載入時是乾淨的確定性倒數）。
- 回復原狀依**行政訴訟法§91**（1 個月、≠民訴§164 的 10 日）。
- 建立回覆會提醒**三個例外須律師另判、系統不自動建**：①§106Ⅰ後段「自訴願決定書送達後逾 3 年不得提起」
  長期失權（另立時鐘）；②**訴願人以外之利害關係人**知悉在後者自知悉時起算（起算改法律判斷、改填知悉日 +
  人工複核）；③不經訴願程序者依§106Ⅲ「自行政處分達到／公告後 2 個月」（起算事件不同、另建一筆並改 trigger）。

```
create_deadline(
  matter_id=<案件ID>, type="admin_revocation",
  service_base_date="2026-02-02",                     # 訴願決定書送達日
  created_by="<操作者>", confirm_intake_id=<intake_id>,
)   # 期間/法條/不變期間性質種子自動帶、不必你填 statutory_days
```

> **訴願 → 行政訴訟接力只做純文字提示 + 一鍵確認、不自動建**（自動判接棒逼近法律判斷）。
