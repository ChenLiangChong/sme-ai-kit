...
2. **一鍵確認才入** — 抽取結果推回 LINE 給律師/助理確認（送達日對不對、是上訴還是抗告、可改）→ **確認後**才 `create_deadline` 寫入。**人必須擋在中間**：律師業算錯期限＝執業過失，不全自動。
...
- **時限**：`create_deadline`（確定性算雙日期）、`get_deadline`（含 calc_trace 逐步覆核）、`list_deadlines`、`list_upcoming_deadlines`（每日彙整/查詢，按內部期限升冪）、`mark_deadline_filed`（已遞交、cron 停提醒）、`mark_deadline_calendared`（回填行事曆 event_id）
