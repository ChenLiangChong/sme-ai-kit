...
db.execute("INSERT INTO inventory (sku, name, current_stock, min_stock, unit, unit_cost, sell_price) VALUES ('SKU-A001', '香氛蠟燭', 50, 20, '個', 200, 500)")
db.execute("INSERT INTO inventory (sku, name, current_stock, min_stock, unit, unit_cost, sell_price) VALUES ('SKU-A002', '擴香瓶', 5, 10, '個', 100, 300)")
db.commit()
db.close()
...
# Threshold test
items_big = json.dumps([{"sku": "SKU-A001", "name": "香氛蠟燭", "qty": 20, "price": 500}])
...
# qc_order 必須在 order.status='confirmed' 之後才能呼叫（L3402-3404 guard）
...
# SKU-A002 had 5 stock, min_stock 10, but was not in this order. SKU-A001 had 50-10=40, min_stock 20, should be ok.
# Actually SKU-A001 50-10=40 > 20, no alert expected for it.
...
items = json.dumps([{"sku": "SKU-A001", "name": "香氛蠟燭", "qty": 2, "price": 425}])
...
# SKU-A002 has 5 stock, min_stock 10 — ordering 3 should trigger alert (5-3=2 < 10)
items = json.dumps([{"sku": "SKU-A002", "name": "擴香瓶", "qty": 3, "price": 300}])
server.create_order(customer_id=1, items_json=items, created_by="test")
db = server.get_db()
...
items = json.dumps([{"sku": "SKU-A001", "name": "香氛蠟燭", "qty": 2, "price": 500}])
