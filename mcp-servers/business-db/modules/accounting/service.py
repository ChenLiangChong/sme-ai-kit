...
def update_transaction(
    ...
    with transaction() as db:
        ...
        if payment_status:
            ...
            elif txn["payment_status"] == "paid":
                # 逆向路徑（codex 複審第二輪殘留 finding）：**從「已付清」轉未付**＝paid_amount 當初是
                # 補滿的全額標記 → 歸零（SQL + new_paid 同步），下方差額重算把這筆對客戶的貢獻負向回沖。
                # 第三輪修正（codex 新 MED）：只在「原本是 paid」時歸零；若本來就是 pending/overdue
                # （paid_amount 可能是 record_payment 累積的部分收款）→ 不碰 paid_amount，避免抹掉既有已收額。
                updates.append("paid_amount = 0")
                new_paid = 0.0
            # else：本來就未付、改 overdue↔pending 之間 → 保留既有 paid_amount（部分收款不動）
