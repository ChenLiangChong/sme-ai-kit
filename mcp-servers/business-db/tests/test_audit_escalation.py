...
_assert("[E-HIGH] 裸 pending 呼 mark_sent（不帶 token）→ 被拒（一律要 token）",
        "不可標記送達" in _res_bare)
...
# === migration 邊界：claimed_at 設了但 claim_token 仍 NULL 的舊 row（legacy 無 token 後門已移除）===
# 第三輪複審 E-HIGH 收尾：一律要 token。無 token 租約（migration 019 前 raw 設 claimed_at 的舊 row）
# 不帶 token 被拒（不再有後門可被任何知道 id 的人標 sent）、帶任意 token 也對不上 NULL → 被拒；
# 仍 pending、留待 TTL 後 cron 以 _CLAIM_UPDATE reclaim（寫新 token）再送（安全延遲、不雙送、不誤清）。
_eid_held = _enqueue("audit·migration 邊界無 token 租約", claimed_at_sql="")  # claimed_at=now、claim_token NULL
_assert("邊界：claimed_at 設了但 claim_token 仍 NULL（模擬 migration 前 raw 設租約）",
        _token_of(_eid_held) is None)
_assert("邊界：無 token 租約 + 不帶 token mark → 被拒（legacy 後門已移除、一律要 token）",
        "不可標記送達" in mark_sent_tool(_eid_held, sent_text="不該過")
        and _status(_eid_held) == "pending")
_assert("邊界：無 token 租約 + 帶任意 token mark → 被拒（對不上 NULL）",
        "未持有此上報的有效租約" in mark_sent_tool(_eid_held, claim_token="someerrtoken", sent_text="x")
        and _status(_eid_held) == "pending")
_assert("邊界：被拒後不落 sent log（無假稽核）", not _has_sent_log(_eid_held))


# === [E-MED] 租約逾 TTL（被 reclaim）後舊持有者不可標 sent（帶對 token 也擋）===
# 帶對 token 但 claimed_at 已逾 TTL → 仍被拒（防 reclaim 後舊持有者雙投）。
...
with transaction() as db:
    db.execute(
        ...
        "claim_token='staletok' WHERE id=?", (_eid_stale,))
_res_stale = mark_sent_tool(_eid_stale, claim_token="staletok", sent_text="逾期不該被接受")
_assert("[E-MED] 租約逾 TTL（帶對 token 也）→ mark_sent 被拒（舊持有者不可標、防雙投）",
...
# 邊界：TTL 內側（TTL-1 分）+ 帶對 token → 仍可標 sent
_eid_edge = _enqueue("audit·TTL 內側邊界可標 sent")
with transaction() as db:
    db.execute(
        f"UPDATE pending_escalations SET claimed_at=datetime('now','localtime','-{_CLAIM_TTL_MIN - 1} minutes'), "
        "claim_token='edgetok' WHERE id=?", (_eid_edge,))
_res_edge = mark_sent_tool(_eid_edge, claim_token="edgetok", sent_text="邊界內送出")
_assert("租約 TTL 內側邊界（TTL-1 分）+ 帶對 token → 仍可標 sent", "已標記 sent" in _res_edge
...
_assert("端到端：有 token 租約但 mark 漏帶 token → 被拒（一律要 token）",
        "不可標記送達" in mark_sent_tool(_eid_e2e, sent_text="漏帶 token 不該過")
