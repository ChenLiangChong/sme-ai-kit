...
# === legal-admin：deadlines 模組（matters + deadlines 計算引擎 + tools）===

_section("Deadlines（legal-admin）")

# create_matter
_rmatter = server.create_matter(
    title="○○公司請求給付貨款",
    matter_no="2026-民-001",
    client_name="○○公司",
    practice_area="civil",
    court="臺灣臺北地方法院",
...
_assert("deadlines: create_matter 建立案件", _has(_rmatter, "案件", "已建立"))
_mid = None
_dbm = server.get_db()
_mrow = _dbm.execute("SELECT id FROM matters WHERE matter_no='2026-民-001'").fetchone()
_dbm.close()
_mid = _mrow[0] if _mrow else 0
_assert("deadlines: matter 落地", _mid > 0)

# 重複案號被擋
_rdup = server.create_matter(title="重複案", matter_no="2026-民-001")
_assert("deadlines: 重複案號被擋", "ERROR" in _rdup or "已存在" in _rdup)

# create_deadline：種子 type 自動回填 statutory_days/basis + 確定性計算落欄
_rdl = server.create_deadline(
    matter_id=_mid,
    type="appeal_civil",
    trigger_event="一審判決送達",
    service_base_date="2026-03-02",
    service_type="normal",
    created_by="王律師",
)
_assert("deadlines: create_deadline（種子回填 + 計算）成功", _has(_rdl, "時限", "已建立"))
_assert("deadlines: 回傳含內部/法定雙日期 + 法條依據",
        _has(_rdl, "內部期限", "法定期限", "民訴§440"))
_assert("deadlines: 回傳含計算軌跡 calc_trace", "計算軌跡" in _rdl)

_dbd = server.get_db()
_drow = _dbd.execute(
    "SELECT id, statutory_days, statutory_basis, statutory_deadline, internal_deadline, "
    "calc_trace, status FROM deadlines WHERE matter_id=? ORDER BY id DESC LIMIT 1", (_mid,)
).fetchone()
_dbd.close()
_did = _drow["id"] if _drow else 0
_assert("deadlines: 種子自動回填 statutory_days=20", _drow and _drow["statutory_days"] == 20)
_assert("deadlines: 種子自動回填 statutory_basis=民訴§440",
        _drow and _drow["statutory_basis"] == "民訴§440")
_assert("deadlines: 計算落欄 statutory_deadline=2026-03-23（golden case）",
        _drow and _drow["statutory_deadline"] == "2026-03-23")
_assert("deadlines: 計算落欄 internal_deadline=2026-03-20（golden case）",
        _drow and _drow["internal_deadline"] == "2026-03-20")
_assert("deadlines: calc_trace 落欄（JSON 陣列、可覆核）",
        _drow and _drow["calc_trace"] and json.loads(_drow["calc_trace"]))

# 非種子 type 缺 statutory_basis → 反捏造擋下（帶 period_type，專測 basis 缺漏這道）
_rbad = server.create_deadline(
    matter_id=_mid, type="custom", trigger_event="送達", service_base_date="2026-06-01",
    description="自訂時限", period_type="peremptory", statutory_days=15,
)
_assert("deadlines: 非種子 type 缺 statutory_basis 被擋（反捏造）",
        "ERROR" in _rbad and "statutory_basis" in _rbad)
# 完全空（無 period_type）也被擋
_rbad2 = server.create_deadline(
    matter_id=_mid, type="custom", trigger_event="送達", service_base_date="2026-06-01",
    description="自訂時限",
)
_assert("deadlines: 非種子 type 缺 period_type 也被擋", "ERROR" in _rbad2)

# get_deadline 含 calc_trace + 救濟備援
_rget = server.get_deadline(_did)
_assert("deadlines: get_deadline 含計算軌跡", _has(_rget, "計算軌跡", "始日不算入"))
_assert("deadlines: get_deadline 含逾期救濟備援", "逾期救濟備援" in _rget)

# list_deadlines
_rlist = server.list_deadlines(matter_id=_mid)
_assert("deadlines: list_deadlines 列出", _has(_rlist, "時限列表", "民訴§440"))

# get_matter 含時限摘要
_rgm = server.get_matter(_mid)
_assert("deadlines: get_matter 含該案時限摘要", _has(_rgm, "案件", "時限"))

# list_matters
_rlm = server.list_matters()
_assert("deadlines: list_matters 列出", _has(_rlm, "案件列表", "2026-民-001"))

# find_matter_by_party：用當事人名字查案件
_rfind = server.find_matter_by_party("○○公司")
_assert("deadlines: find_matter_by_party 查到案件", _has(_rfind, "案件查詢", "2026-民-001"))
_rfind_none = server.find_matter_by_party("查無此人ABCXYZ")
_assert("deadlines: find_matter_by_party 查無回提示", "找不到" in _rfind_none)
_rfind_empty = server.find_matter_by_party("")
_assert("deadlines: find_matter_by_party 空名擋下", "ERROR" in _rfind_empty)

# list_upcoming_deadlines：pending 按內部期限升冪（此時 _did 仍 pending）
_rup = server.list_upcoming_deadlines()
_assert("deadlines: list_upcoming_deadlines 列出 pending 含雙日期+法條",
...
_dbf = server.get_db()
_fstatus = _dbf.execute("SELECT status FROM deadlines WHERE id=?", (_did,)).fetchone()
_dbf.close()
_assert("deadlines: 遞交後狀態=filed（cron 不再提醒）", _fstatus and _fstatus[0] == "filed")
# 重複遞交被擋
_rrefiled = server.mark_deadline_filed(_did, filed_by="王律師")
_assert("deadlines: 已遞交時限重複標記被擋", "ERROR" in _rrefiled)

# scan_and_enqueue_due_reminders：對「內部期限=今天」的 pending deadline 應 enqueue
from datetime import date as _date  # noqa: E402
from shared.deadlines import scan_and_enqueue_due_reminders  # noqa: E402
# 建一筆內部期限剛好=今天的 pending deadline（lead_day 0 命中）
_dbs = server.get_db()
_today = _date.today().isoformat()
_dbs.execute(
    "INSERT INTO deadlines (matter_id, type, description, period_type, severity, trigger_event, "
    "service_type, service_base_date, statutory_days, statutory_basis, statutory_deadline, "
    "internal_deadline, buffer_days, escalation_lead_days, reminders_sent, status) "
    "VALUES (?, 'custom', 'scan 測試', 'peremptory', 'red', '送達', 'normal', '2026-01-01', 20, "
    "'民訴§440', ?, ?, 0, '[0]', '[]', 'pending')",
    (_mid, _today, _today),
)
_dbs.commit()
_scanrow = _dbs.execute(
    "SELECT id FROM deadlines WHERE description='scan 測試'"
).fetchone()
_dbs.close()
_scanstats = scan_and_enqueue_due_reminders(today=_today)
_assert("deadlines: scan_and_enqueue 掃到 pending 並 enqueue approaching",
        _scanstats["approaching"] >= 1, detail=str(_scanstats))
_dbs2 = server.get_db()
_escrow = _dbs2.execute(
    "SELECT event_type, actor FROM pending_escalations "
    "WHERE event_type='deadline_approaching' ORDER BY id DESC LIMIT 1"
).fetchone()
_sent_after = _dbs2.execute(
    "SELECT reminders_sent FROM deadlines WHERE id=?", (_scanrow[0],)
).fetchone()
_dbs2.close()
_assert("deadlines: enqueue 了 deadline_approaching 事件（接現役三層投遞）",
        _escrow is not None and _escrow["event_type"] == "deadline_approaching")
_assert("deadlines: scan 以系統蓋章 actor（非人）", _escrow and "系統" in (_escrow["actor"] or ""))
_assert("deadlines: reminders_sent 冪等鑰回寫（lead_day 0 已記）",
        _sent_after and 0 in json.loads(_sent_after[0]))
# 同日重掃不重推（冪等）
_scanstats2 = scan_and_enqueue_due_reminders(today=_today)
_assert("deadlines: 同日重掃冪等不重推（reminders_sent 去重）",
        _scanstats2["approaching"] == 0, detail=str(_scanstats2))


# === HIGH-1（codex）：deadlines mutator 機密軸寫入 gate ===
# 修前：受限層 create_deadline / mark_deadline_filed 對機密案件【無權限檢查】→ 可對看不到的機密
...
_assert("H1: 機密案件建立（全權限路徑）", _has(_rcm, "案件", "已建立"))
_dbh1 = server.get_db()
_cmid = _dbh1.execute("SELECT id FROM matters WHERE matter_no='2026-密-001'").fetchone()[0]
_dbh1.close()
# 先用全權限建一筆其下的時限（供後續測 mark_deadline_filed gate）
_rch_dl = server.create_deadline(
    matter_id=_cmid, type="appeal_civil", trigger_event="一審判決送達",
    service_base_date="2026-03-02", service_type="normal", created_by="老闆",
)
_assert("H1: 全權限可對機密案件建時限（baseline）", _has(_rch_dl, "時限", "已建立"))
_dbh1b = server.get_db()
_cdid = _dbh1b.execute(
    "SELECT id FROM deadlines WHERE matter_id=? AND status='pending' ORDER BY id DESC LIMIT 1",
    (_cmid,)).fetchone()[0]
_cnt_before = _dbh1b.execute(
...
try:
    ...
    _cnt_after = _dbh1c.execute(
        "SELECT COUNT(*) FROM deadlines WHERE matter_id=?", (_cmid,)).fetchone()[0]
    _dbh1c.close()
    _assert("H1: 受限層 create_deadline 被擋後【未寫入】時限",
            _cnt_after == _cnt_before, detail=f"before={_cnt_before} after={_cnt_after}")
    ...
    _dbh1f.close()
    _assert("H1: 受限層被擋後時限狀態仍 pending（未被標 filed）",
...
try:
    ...
    _assert("H1: 受限層對【非機密】案件 create_deadline 照常可寫（gate 不過度收斂）",
...
# 修前：scan 把 assignee_line_user_id 塞進 channel_id（OA 欄）→ 送錯人 + flusher 拿它查 token fail。
# 修後：收件人走 resolve_escalation_target（boss/全所），channel_id 不被 assignee user_id 污染；
# 承辦律師資訊出現在 summary。
_section("Deadlines HIGH-2：將至提醒收件人=全所、channel_id 不污染")

# 種一個 boss 員工當全所收件人
server.register_employee(name="律所老闆H2", role="boss",
                         line_user_id="Uboss_h2_000000000000000000000000", permissions="admin")
# 建一筆「內部期限=今天、有 assignee_line_user_id（承辦律師個人 id）」的 pending deadline
_dbh2 = server.get_db()
_today_h2 = _date.today().isoformat()
_dbh2.execute(
    "INSERT INTO matters (matter_no, title, status, has_local_agent, lead_attorney, confidential) "
    "VALUES ('2026-H2-001', 'H2 收件人測試案', 'open', 1, '陳律師', 0)"
)
_h2mid = _dbh2.execute("SELECT id FROM matters WHERE matter_no='2026-H2-001'").fetchone()[0]
_assignee_uid_h2 = "Uassignee_h2_00000000000000000000"  # 承辦律師個人 id（修前會被當 channel_id）
_dbh2.execute(
    "INSERT INTO deadlines (matter_id, type, description, period_type, severity, trigger_event, "
    "service_type, service_base_date, statutory_days, statutory_basis, statutory_deadline, "
    "internal_deadline, buffer_days, assignee, assignee_line_user_id, escalation_lead_days, "
    "reminders_sent, status) "
    "VALUES (?, 'custom', 'H2上訴狀補提', 'peremptory', 'red', '送達', 'normal', '2026-01-01', 20, "
    "'民訴§471', ?, ?, 0, '陳律師', ?, '[0]', '[]', 'pending')",
    (_h2mid, _today_h2, _today_h2, _assignee_uid_h2),
)
_dbh2.commit()
_dbh2.close()

_scan_h2 = scan_and_enqueue_due_reminders(today=_today_h2)
_assert("H2: scan enqueue 將至提醒", _scan_h2["approaching"] >= 1, detail=str(_scan_h2))
_dbh2r = server.get_db()
_eh2 = _dbh2r.execute(
    "SELECT summary, target_line_user_id, channel_id, detail FROM pending_escalations "
    "WHERE event_type='deadline_approaching' AND summary LIKE '%H2上訴狀補提%' "
    "ORDER BY id DESC LIMIT 1"
).fetchone()
_dbh2r.close()
_assert("H2: 將至提醒收件人解析為全所 boss（非承辦律師個人）",
        _eh2 is not None and _eh2["target_line_user_id"] == "Uboss_h2_000000000000000000000000",
        detail=str(dict(_eh2)) if _eh2 else "None")
_assert("H2: channel_id【不被】承辦律師 user_id 污染（OA 欄語意正確）",
        _eh2 is not None and _eh2["channel_id"] != _assignee_uid_h2,
        detail=f"channel_id={_eh2['channel_id'] if _eh2 else None}")
_assert("H2: 承辦律師資訊出現在 summary（讓全所看得到承辦）",
        _eh2 is not None and "承辦：陳律師" in (_eh2["summary"] or ""),
        detail=_eh2["summary"] if _eh2 else "None")
_assert("H2: detail 也帶 assignee 欄（全字串、供稽核）",
        _eh2 is not None and json.loads(_eh2["detail"]).get("assignee") == "陳律師",
