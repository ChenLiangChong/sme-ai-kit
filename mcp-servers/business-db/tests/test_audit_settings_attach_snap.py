...
# A6：list_attachments 未知 target_type → ERROR（codex 複審第二輪殘留：list 對未知 type 靜默回空）
_a6 = asvc.list_for(target_type="employee", target_id=1)
_assert("A6: list 未知 target_type → ERROR（白名單外擋下）",
        _a6.startswith("ERROR") and "不支援" in _a6, detail=_a6[:120])

# A7：list 合法 target_type（task）→ 回該 task 附件（A1 已對 _TASK_ID 加一個）、非 ERROR
_a7 = asvc.list_for(target_type="task", target_id=_TASK_ID)
_assert("A7: list 合法 target_type 正常回附件（非 ERROR）",
        not _a7.startswith("ERROR") and "a.pdf" in _a7, detail=_a7[:160])

# A8：list 合法 target_type 但該 target 無附件 → 回空訊息（非 ERROR、存在性不強檢）
_a8 = asvc.list_for(target_type="customer", target_id=999999)
_assert("A8: 合法 target_type 無附件 → 非 ERROR（空列表合法）",
        not _a8.startswith("ERROR") and "沒有附件" in _a8, detail=_a8[:120])
...
# ============================================================
# settings [MED/HIGH]：save_session_handoff / resolve_handoff actor fail-closed
# （codex 複審第二輪殘留：受限層無 verified 可污染跨 session 控制面）
# ============================================================

# H1：operator（無 SME_FLOOR、空 actor）放行 → 存 handoff
_h1 = ssvc.save_handoff(session_id="sess-operator-abc", summary="operator 交接", pending_items="[]",
                        actor_user_id="")
_hid1 = _id(_h1)
_assert("H1: operator 路徑存 handoff 放行", _hid1 is not None and "已儲存" in _h1, detail=_h1)
_assert("H1: handoff 確實寫入（active）",
        _q1("SELECT status FROM session_handoffs WHERE id=?", (_hid1,))["status"] == "active")
# 留 audit
_log_h1 = _q1("SELECT actor FROM interaction_log WHERE action='handoff_saved' AND target_id=?", (_hid1,))
_assert("H1: save 留 audit（handoff_saved）", _log_h1 is not None, detail=str(dict(_log_h1)) if _log_h1 else "none")

# H2：floored 無 verified LINE 脈絡（沒寫 active-request）→ save 擋下、不寫入
_n_before = _q1("SELECT COUNT(*) c FROM session_handoffs")["c"]
_h2 = _with_floor("general", lambda: ssvc.save_handoff(
    session_id="sess-floored-noverify", summary="想污染", pending_items="[]",
    actor_user_id=""), user_id=None)
_n_after = _q1("SELECT COUNT(*) c FROM session_handoffs")["c"]
_assert("H2: floored 無 verified → save 擋下（ERROR）", _h2.startswith("ERROR"), detail=_h2[:120])
_assert("H2: 被擋後未寫入 handoff（fail-closed 在寫入前）", _n_before == _n_after, detail=f"{_n_before}->{_n_after}")

# H3：floored verified 員工（basic 也可存自己的 handoff、不需 manager）→ 放行、actor 具名
_h3 = _with_floor("general", lambda: ssvc.save_handoff(
    session_id="sess-floored-verified", summary="受限層自己的交接", pending_items="[]",
    actor_user_id="agent亂填"), user_id="Ubasic_test")
_hid3 = _id(_h3)
_assert("H3: floored verified（basic）存自己的 handoff 放行", _hid3 is not None and "已儲存" in _h3, detail=_h3)
_log_h3 = _q1("SELECT actor FROM interaction_log WHERE action='handoff_saved' AND target_id=?", (_hid3,))
_assert("H3: audit actor 用 verified 員工名（小李、非 agent 自填）",
        _log_h3 and _log_h3["actor"] == "小李", detail=str(dict(_log_h3)) if _log_h3 else "none")

# H4：resolve_handoff floored 無 verified → 擋下、handoff 仍 active（未被偷標 resolved）
_h4_target = _hid1  # 用 H1 存的 operator handoff 當靶
_h4 = _with_floor("general", lambda: ssvc.resolve_handoff(
    handoff_id=_h4_target, note="想隱藏", actor_user_id=""), user_id=None)
_assert("H4: resolve floored 無 verified → 擋下（ERROR）", _h4.startswith("ERROR"), detail=_h4[:120])
_assert("H4: 被擋後 handoff 仍 active（未被偷標 resolved）",
        _q1("SELECT status FROM session_handoffs WHERE id=?", (_h4_target,))["status"] == "active")

# H5：resolve_handoff operator（空 actor）放行 + 留 audit
_h5 = ssvc.resolve_handoff(handoff_id=_h4_target, note="接手完成", actor_user_id="")
_assert("H5: operator resolve 放行", "已標記 resolved" in _h5, detail=_h5)
_assert("H5: handoff 確實標 resolved",
        _q1("SELECT status FROM session_handoffs WHERE id=?", (_h4_target,))["status"] == "resolved")
_log_h5 = _q1("SELECT actor FROM interaction_log WHERE action='handoff_resolved' AND target_id=?", (_h4_target,))
_assert("H5: resolve 留 audit（handoff_resolved）", _log_h5 is not None, detail=str(dict(_log_h5)) if _log_h5 else "none")
