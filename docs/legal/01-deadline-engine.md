...
**消滅時效（年/月期間，`type='limitation'` / `LIMITATION_PERIODS`，migration 015 加 `period_unit`/`period_value`）**：與訴訟「日數」期間根本不同。期間是「年」（民§125=15 / §126=5 / §127=2 / §197=2+10），依民§121「以年/月定期間→以最後之年/月與起算日『相當日之前一日』為末日；無相當日（閏日/月末）→該月末日」+ §123「連續計算依曆」，**不可硬轉天數**（閏年會差、反捏造；自實作 `_statute_period_end`、不引入 dateutil）。`compute_deadline` 走 `counting_regime='limitation'` 分支：無在途、無送達加算（民§128 自請求權可行使時直接起算）、不適用回復原狀。起算點是民§128「請求權可行使時」＝**法律判斷**（非送達日這種確定事實；§197 侵權還含主觀「知悉時」）→ `create_deadline` 對消滅時效一律強制 `needs_manual_review`、起算日（`service_base_date`）由律師輸入。§122 末日順延於消滅時效見解分歧 → 引擎不臆測順延、依曆末日為準（爭議包進強制複核、律師個案決定）。§197 雙時鐘（知悉2年 `statute_197_2y` + 行為10年 `statute_197_10y`）各建一筆、不自動雙建（避免系統替律師判斷「知悉時」）。`period_type` 用 `statutory`，靠 `counting_regime` 而非 `period_unit` 觸發時效 regime。

**保全命起訴期間（`type='provisional_litigation'` / `COURT_SET_PERIODS`）**：假扣押/假處分本案未繫屬時，法院依債務人聲請命債權人「於『一定期間』內起訴」（民訴§529Ⅰ，假處分準用§533）。**期間是法院於命起訴裁定主文當下所定、非法定固定日數**（坊間「30日」是慣例非法律值＝捏造風險）→ 歸 court_set：律師讀那紙命起訴裁定填天數、引擎不臆測不預設、強制 `needs_manual_review`。severity=red（逾期未起訴→債務人得聲請撤銷保全裁定·§529Ⅳ/§533＝保全盡失、無補正餘地，較限期補正更失權）。建立回覆提醒特例§529Ⅲ（夫妻剩餘財產差額分配假扣押「裁定確定之日」起10日內起訴、起算為「裁定確定」非送達、亦走 court_set 手填）。

**程序月期間（`type='admin_revocation'` / `PROCEDURAL_CALENDAR_PERIODS`，行訴§106Ⅰ）**：行政訴訟撤銷訴訟「應於訴願決定書送達後**二個月之不變期間**內」提起。與消滅時效都是「月」期間、但 regime 不同——**走程序機制**（`counting_regime='procedural'`）：送達生效→民§120Ⅱ次日起算→依曆2個月（民§121/§123，行訴§88「期間之計算依民法」）→在途（行訴§89）→§122末日順延。起算用「訴願決定書送達日」這個**確定事實**（非§128 法律判斷）→ **不**強制複核（這正是相對消滅時效可確定性自動算之處）。回復原狀依**行政訴訟法§91**（原因消滅後**1個月**內、逾1年或§106起訴逾3年不得聲請；≠民訴§164 之10日，由 `statutory_basis` 偵測行政案自動標對法條）。建立回覆提醒三例外須律師另判：①§106Ⅰ後段「自訴願決定書送達後逾3年不得提起」長期失權（另立時鐘）、②利害關係人知悉在後者自知悉時起算（起算改法律判斷）、③不經訴願者§106Ⅲ「自行政處分達到/公告後2個月」（起算事件不同、另建並改 trigger）。引擎用兩條正交軸 `_is_calendar`（年/月→§121曆法）與 `_is_limitation`（`counting_regime`→消滅時效 regime）解耦，月期間因此可同時是程序不變期間（行訴§106）或實體時效（民§125）。
