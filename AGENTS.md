...
>
> 兩檔內容**必須一致**。修改後執行 `cp CLAUDE.md AGENTS.md` 同步。
...
## MCP 工具
- **business-db**：企業資料庫（51 tools — 知識/任務/員工/客戶/庫存/帳務/訂單/審核/附件/快照/公司設定/事業體/LINE 訊息搜尋/LINE 群組）
- **line**：LINE Channel（5 tools — reply/reply_flex/multicast/mark_read/list_channels）
...
3. **先辨識身份**：員工 → 客戶 → 暱稱比對 → 陌生人分層路由
4. **陌生人不直接回覆**，依意圖路由通知對應負責人（詳見 line-comms.md 第二節）
5. **每則訊息必須有結局**：`reply`（已回覆）或 `mark_read`（已處理）
6. **對外行銷訊息需 HITL 審核**
...
核心邏輯：
1. **檢查 LINE 是否設定好**：嘗試呼叫任一 line tool（如 `list_channels`）
